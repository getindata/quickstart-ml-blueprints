...
class GcpHMDataset(Dataset):
    ...
    def _get_img_names(self):
        blobs = self.storage_client.list_blobs(
            self.bucket_name, prefix=self.data_folder
        )
        img_names = [
            blob.name.split("/")[-1] for blob in blobs if blob.name[-3:] == "jpg"
        ]
        return img_names
...
class HMDataLoader(pl.LightningDataModule):
    def __init__(
        ...
        self.pl_dataset = {"local": LocalHMDataset, "gcp": GcpHMDataset}.get(
            self._platform
        )
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            ...
            self.hm_train, self.hm_val = random_split(
                hm_dataset, [train_size, val_size]
            )
