...
# COPIED FROM
# https://gitlab.com/getindata/internal/aa_team_kagle_hm/-/blob/main/src/vertex_ai_plugin_demo/image_embeddings/model/decoders.py
...
class DecoderConvCompr(nn.Module):
    def __init__(self, embedding_size: int = 32, image_size: List[int] = [128, 128]):
        ...
        self.unflatten = nn.Unflatten(dim=1, unflattened_size=(32, 32, 32))
...
class DecoderConvCompr3LayerV3(nn.Module):
    def __init__(self, embedding_size: int = 32, image_size: List[int] = [128, 128]):
        ...
        self.decoder_lin = nn.Sequential(
            nn.Linear(self.embedding_size, 8192), nn.ReLU()
        )
...
class DecoderConvLarger1stKernel(nn.Module):
    def __init__(self, embedding_size: int = 32, image_size: List[int] = [128, 128]):
        ...
        self.decoder_lin = nn.Sequential(
            nn.Linear(self.embedding_size, 7200), nn.ReLU()
        )
...
class DecoderConvLarger2KernelsLeaky(nn.Module):
    def __init__(self, embedding_size: int = 32, image_size: List[int] = [128, 128]):
        ...
        self.decoder_lin = nn.Sequential(
            nn.Linear(self.embedding_size, 7200), nn.LeakyReLU()
        )
