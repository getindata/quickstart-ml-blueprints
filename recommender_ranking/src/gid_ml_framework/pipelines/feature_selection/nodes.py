...
def _remove_correlated_features(
    df: pd.DataFrame, corr_threshold: float = 0.99
) -> pd.DataFrame:
    ...
    for col in col_list:
        ...
        corr_cols_list = corr_matrix.index[
            corr_matrix.loc[:, col].ge(corr_threshold)
        ].to_list()
        if len(corr_cols_list) > 0:
            correlated_columns |= set(corr_cols_list)
    logger.info(
        f"""
        Correlated: {len(correlated_columns)=}, \n
...
def feature_selection(
    df: pd.DataFrame, selection_params: Dict, feature_selection: bool
) -> Set:
    """Applies multiple feature_selection functions to a dataframe:
    highly null values, single value features, correlated features.
    ...
    df = remove_highly_null_features(
        df, pct_null_threshold=selection_params["pct_null_threshold"]
    )
    df = remove_single_value_features(df)
    df = _remove_correlated_features(
        df, corr_threshold=selection_params["corr_threshold"]
    )
    logger.info(f"Shape after feature selection: {df.shape}")
    feature_selection_cols = set(df.columns)
    logger.info(
        f"Number of removed cols: {len(initial_cols.difference(feature_selection_cols))}"
    )
