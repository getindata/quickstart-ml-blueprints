...
def _get_recall(
    candidates: pd.DataFrame, val_transactions: pd.DataFrame, candidate_col: str
) -> float:
    ...
    """
    exploded_candidates_df = candidates[["customer_id", candidate_col]].explode(
        candidate_col
    )
...
def log_retrieval_recall(
    candidates: pd.DataFrame, val_transactions: pd.DataFrame
) -> None:
