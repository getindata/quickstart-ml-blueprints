...
def filter_dataframe_by_last_n_weeks(
    df: pd.DataFrame, date_column: str, no_weeks: int = 6
) -> pd.DataFrame:
    ...
    """
    if not no_weeks:
        logger.info("no_weeks is equal to None, skipping the filtering by date step.")
        return df
    try:
    ...
    logger.info(
        f"Before filtering: dataframe min date: {df[date_column].min()}, max date: {max_date}"
    )
    min_date = max_date - pd.Timedelta(weeks=no_weeks)
    df = df[df[date_column] > min_date]
    logger.info(
        f"After filtering: dataframe min date: {df[date_column].min()}, max date: {max_date}"
    )
    return df
