...
def _concat_dataframes_on_index(
    list_of_dfs: List[pd.DataFrame], index_name: Union[str, List[str]]
) -> pd.DataFrame:
...
def _rebuying_articles(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df_rebought = (
        transactions.groupby(["customer_id", "article_id"])["t_dat"]
        .count()
        .reset_index()
...
def create_article_features(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    df_result = _concat_dataframes_on_index(
        [df_rebought, df_mean_sales_channel_id], "article_id"
    )
    return df_result


# CUSTOMER FEATURES
def _count_of_article_id_per_customer_id(
    transactions: pd.DataFrame, n_days: Optional[int]
) -> pd.DataFrame:
    ...
    """
    filtered_transactions = filter_dataframe_by_last_n_days(
        transactions, n_days, date_column="t_dat"
    )
    df_count_articles = (
        filtered_transactions.groupby(["customer_id"])["article_id"]
        .count()
...
def _count_of_product_group_name_per_customer_id(
    transactions: pd.DataFrame, articles: pd.DataFrame, n_days: Optional[int]
) -> pd.DataFrame:
...
def _days_since_first_transactions(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    df_days_since_first = (
        max_date - (transactions.groupby(["customer_id"])["t_dat"].min())
...
def _days_since_last_transactions(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    max_date = transactions["t_dat"].max()
    df_days_since_last = (
        max_date - (transactions.groupby(["customer_id"])["t_dat"].max())
...
def _average_purchase_span(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    sorted_transactions["customer_id_next"] = sorted_transactions["customer_id"].shift(
        1
    )
    sorted_transactions = sorted_transactions.assign(
        avg_purchase_span=lambda x: x.t_dat_next - x.t_dat
    )
    df_purchase_span = (
        sorted_transactions[
            sorted_transactions["customer_id"]
            == sorted_transactions["customer_id_next"]
        ]
...
def create_customer_features(
    transactions: pd.DataFrame, articles: pd.DataFrame, n_days_list: List[Optional[int]]
) -> pd.DataFrame:
...
def _count_of_article_id_per_customer_product_group(
...
def create_customer_product_group_features(
    transactions: pd.DataFrame, articles: pd.DataFrame
) -> pd.DataFrame:
    ...
    df_count_article = _count_of_article_id_per_customer_product_group(
        transactions, articles
    )
    # concat
    logger.info("Concatenating customer x product_group_name features...")
    df_result = _concat_dataframes_on_index(
        [df_count_article], ["customer_id", "product_group_name"]
    )
    return df_result
