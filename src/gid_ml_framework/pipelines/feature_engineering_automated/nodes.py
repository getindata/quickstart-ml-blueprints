...
def _create_entity_set(transactions: pd.DataFrame, customers: pd.DataFrame, articles: pd.DataFrame, n_days: int) -> ft.EntitySet:
    ...
    transactions = filter_dataframe_by_last_n_days(transactions, n_days, date_column='t_dat')
    # transactions
    # needs unique (!) index
...
def create_static_features(
    ...
    for n_days in n_days_list:
        suffix_str_article = '_all_articles_automated' if n_days is None else f'_{n_days}_articles_automated'
        suffix_str_customer = '_all_customers_automated' if n_days is None else f'_{n_days}_customers_automated'
