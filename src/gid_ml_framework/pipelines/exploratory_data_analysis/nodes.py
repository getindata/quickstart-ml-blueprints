...
def _price_per_product_groups(articles: pd.DataFrame, transactions: pd.DataFrame) -> None:
    # join
    transactions_articles = transactions[['article_id', 'price']].merge(articles[['article_id', 'product_group_name', 'index_name']], on='article_id', how='left')
    # plot
    ...
    plt.close()
    # plot 2
    df = transactions_articles[['product_group_name', 'price']].groupby(['product_group_name'])['price'].mean().reset_index()
    sns.set_style("darkgrid")
    ...
    plt.close()
    # plot 3
    df = transactions_articles[['index_name', 'price']].groupby(['index_name'])['price'].mean().reset_index()
    sns.set_style("darkgrid")
    fig, ax = plt.subplots(figsize=(10,5))
