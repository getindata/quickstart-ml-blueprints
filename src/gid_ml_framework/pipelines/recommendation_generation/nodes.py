...
def _weight_blend(predictions: pd.Series, prediction_cols: List[str], k: int = 12) -> str:
...
def _ensemble_predictions(predictions: pd.DataFrame, k: int = 12) -> pd.DataFrame:
    ...
    Args:
        predictions (pd.DataFrame): predictions dataframe with multiple prediction columns (col name must start with `prediction_`)
        k (int, optional): top k recommended items. Defaults to 12.
...
def _predict_from_multiple_models(candidates: pd.DataFrame, models: List[str], k: int = 12) -> pd.DataFrame:
...
def _predict_from_single_model(candidates: pd.DataFrame, model_str: str, k: int = 12) -> pd.DataFrame:
...
def generate_predictions(candidates: pd.DataFrame, models: List[str], k: int = 12) -> pd.DataFrame:
